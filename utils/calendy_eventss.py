...
startTime = dt.replace(day=9, hour=6, minute=0, second=0, microsecond=0).strftime('%Y-%m-%dT%H:%M:%S.%fZ')
...
def getEventDetail(uuid):
    conn = http.client.HTTPSConnection("api.calendly.com")

    headers = { 'authorization': "Bearer eyJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL2F1dGguY2FsZW5kbHkuY29tIiwiaWF0IjoxNjIyNTgwMTU5LCJqdGkiOiJjZjZjMTEzNC02NGZhLTRkNTEtYjM0MC1kZWZmMDBkODlhNTciLCJ1c2VyX3V1aWQiOiJFQ0dHUVU2WTVaRldZRkVIIn0.8Xm2RsfezOsnB4Qc6WQ_tDectplwICeUX1WIujCq6Kw" }
    url = "/scheduled_events/" + uuid + "/invitees"
...
def getEvents():
    data = agenda()
    mensagem = []
    data = datetime.strptime(data['collection'][0]['start_time'], '%Y-%m-%dT%H:%M:%S.%fz')
    for i in data['collection']:
        uri = i["uri"]
        detail = getEventDetail(uri[-16:])
        mensagem.append("nome : " + detail['collection'][0]["questions_and_answers"][0]["answer"])
